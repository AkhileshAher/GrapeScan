...
function PredictionRow({ prediction }) {
  return (
    <tr className="border-b border-slate-100 dark:border-slate-850 last:border-0 hover:bg-slate-50/60 dark:hover:bg-slate-850/40 transition-colors">
      ...
      <td className="py-3 px-4 text-sm text-slate-500 dark:text-slate-400">{formatDate(prediction.date)}</td>
    </tr>
  );
}
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center text-center py-14 px-4">
      ...
      <h4 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-1">No predictions yet</h4>
      <p className="text-slate-500 dark:text-slate-400 mb-5">Upload your first grape leaf image to get started!</p>
      <Link
        to="/predict-page"
...
function ErrorState({ onRetry }) {
  return (
    <div className="flex flex-col items-center text-center py-14 px-4">
      ...
      <p className="text-slate-600 dark:text-slate-400 mb-4">Failed to load prediction history.</p>
      <button
        onClick={onRetry}
        className="inline-flex items-center gap-2 bg-slate-100 dark:bg-slate-805 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-medium px-5 py-2.5 rounded-xl transition-colors"
      >
        🔄 Retry
      </button>
    </div>
...
export default function PredictionHistory({ predictions, status, onRetry }) {
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-850 shadow-sm p-5 sm:p-6">
      <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-4">📋 Prediction History</h3>
